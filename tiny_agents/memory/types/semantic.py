...
class SemanticMemory(BaseMemory):
    ...
    def retrieve(self, query: str, limit: int = 5, **kwargs) -> List[MemoryItem]:
        ...
        try:
            ...
            if debug:
                logger.info(f"🐛 [DEBUG] retrieve() 开始: query='{query}', limit={limit}, user_id='{user_id}'")

            # 1. 向量检索
            if debug:
                logger.info(f"🐛 [DEBUG] 开始向量检索...")
            vector_results = self._vector_search(query, limit * 2, user_id)
            if debug:
                logger.info(f"🐛 [DEBUG] 向量检索返回 {len(vector_results)} 个结果")

            # 2. 图检索
            if debug:
                logger.info(f"🐛 [DEBUG] 开始图检索...")
            graph_results = self._graph_search(query, limit * 2, user_id)
            if debug:
                logger.info(f"🐛 [DEBUG] 图检索返回 {len(graph_results)} 个结果")

            # 3. 混合排序
            if debug:
                logger.info(f"🐛 [DEBUG] 开始混合排序...")
            combined_results = self._combine_and_rank_results(
                vector_results, graph_results, query, limit
            )
            if debug:
                logger.info(f"🐛 [DEBUG] 混合排序返回 {len(combined_results)} 个结果")
            ...
            if debug:
                logger.info(f"🐛 [DEBUG] 过滤掉 {filtered_count} 条已遗忘记忆")
                logger.info(f"🐛 [DEBUG] 最终返回 {len(result_memories)} 条记忆")
    ...
    def _vector_search(self, query: str, limit: int, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"🔍 [向量搜索] 集合信息: points={collection_info.get('points_count', 0)}, vector_size={collection_info.get('config', {}).get('vector_size', 'unknown')}")
            except Exception as ie:
                logger.warning(f"🔍 [向量搜索] 无法获取集合信息: {ie}")
            ...
            if results:
                for i, r in enumerate(results[:3]):  # 只打印前3个结果
                    logger.info(f"  - 结果{i+1}: id={r.get('id')}, score={r.get('score'):.4f}, memory_id={r.get('metadata', {}).get('memory_id')}, user_id={r.get('metadata', {}).get('user_id')}")
            ...
            logger.info(f"✅ [向量搜索] 完成查询: 返回 {len(formatted_results)} 个格式化结果")
    ...
    def _extract_entities_by_rules(self, text: str) -> List[Entity]:
        ...
        # 1. 优先提取人名（上下文匹配，更精确）
        seen_names = set()
        name_keywords = ['我叫', '我的名字是', '我是', '名字叫', '的名字是']
        for kw in name_keywords:
            if kw in text:
                idx = text.find(kw)
                after_kw = text[idx + len(kw):idx + len(kw) + 15]
                name_match = re.search(r'[一-龥]{2,4}', after_kw)
                if name_match:
                    name = name_match.group()
                    skip_words = ['人工', '智能', '内容', '产品', '技术', '一名', '名内', '名容', '名产']
                    if (name not in seen_names and 
                        len(name) >= 2 and 
                        not any(skip in name for skip in skip_words)):
                        entity = Entity(
                            entity_id=f"rule_person_{hash(name)}",
                            name=name,
                            entity_type="PERSON",
                            description="规则提取的人名"
                        )
        ...
        # 2. 提取技能/技术关键词
        skill_patterns = [
            r'LLM', r'AI', r'人工智能', r'机器学习', r'深度学习',
            r'Python', r'Java', r'JavaScript', r'前端', r'后端'
        ]
        for pattern in skill_patterns:
            if pattern in text:
                entity = Entity(
                    entity_id=f"rule_skill_{hash(pattern)}",
                    name=pattern,
                    entity_type="SKILL",
                    description="规则提取的技能"
                )
        ...
        # 3. 提取职业角色
        role_patterns = [
            r'产品经理', r'工程师', r'设计师', r'开发者', 
            r'专家', r'爱好者', r'创作者', r'学生'
        ]
        for pattern in role_patterns:
            if pattern in text:
                entity = Entity(
                    entity_id=f"rule_role_{hash(pattern)}",
                    name=pattern,
                    entity_type="ROLE",
                    description="规则提取的角色"
                )
