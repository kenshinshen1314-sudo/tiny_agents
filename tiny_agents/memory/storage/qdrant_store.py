...
class QdrantConnectionManager:
    ...
    def get_instance(
        ...
        collection_name: str = "tiny_agents_vectors",
        vector_size: int = 384,
        distance: str = "cosine",
        timeout: int = 30,
        **kwargs
...
class QdrantVectorStore:
    ...
    def search_similar(
        ...
        try:
            ...
            # 优先使用新版 API (query_points)，回退到旧版 (search)
            search_result = None
            if hasattr(self.client, 'query_points'):
                # 新版 Qdrant 客户端 (>= v1.7.0)
                ...
                raise AttributeError("QdrantClient 缺少 search 或 query_points 方法")
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 兼容新版 API - 使用 config.params
            points_count = 0
            segments_count = 0

            # 新版 API 使用 points_count
            if hasattr(collection_info, 'points_count'):
                points_count = collection_info.points_count
            elif hasattr(collection_info, 'config') and hasattr(collection_info.config, 'params'):
                points_count = collection_info.config.params.points_count

            # segments_count
            if hasattr(collection_info, 'segments_count'):
                segments_count = collection_info.segments_count
            elif hasattr(collection_info, 'config') and hasattr(collection_info.config, 'optimizer_status'):
                # 新版 API 可能不提供 segments_count
                segments_count = 0

            info = {
                "name": self.collection_name,
                "vectors_count": points_count,  # 使用 points_count
                "indexed_vectors_count": points_count,  # 使用 points_count
