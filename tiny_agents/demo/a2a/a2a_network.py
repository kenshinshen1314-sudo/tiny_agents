...
from tiny_agents.protocols import A2AServer, A2AClient
import threading
import time
...
# 2. 启动所有服务
threading.Thread(target=lambda: researcher.run(port=5123), daemon=True).start()
threading.Thread(target=lambda: writer.run(port=5121), daemon=True).start()
threading.Thread(target=lambda: editor.run(port=5122), daemon=True).start()
time.sleep(2)  # 等待服务启动

# 3. 创建客户端连接到各个Agent
researcher_client = A2AClient("http://localhost:5123")
writer_client = A2AClient("http://localhost:5121")
editor_client = A2AClient("http://localhost:5122")
