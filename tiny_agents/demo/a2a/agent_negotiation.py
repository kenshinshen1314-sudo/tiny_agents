...
if __name__ == "__main__":
    threading.Thread(target=lambda: agent1.run(port=6000), daemon=True).start()
    threading.Thread(target=lambda: agent2.run(port=6001), daemon=True).start()
    time.sleep(2)
    
    # 测试协商流程
    client1 = A2AClient("http://localhost:6000")
    client2 = A2AClient("http://localhost:6001")
