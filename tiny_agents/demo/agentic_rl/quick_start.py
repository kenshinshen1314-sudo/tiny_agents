...
def quick_test():
    ...
    if sft_data.get("status") != "success":
        print(f"\n❌ SFT训练失败: {sft_data.get('message', '未知错误')}")
        return
    print("\n✅ SFT训练结果:")
    ...
    if grpo_data.get("status") != "success":
        print(f"\n❌ GRPO训练失败: {grpo_data.get('message', '未知错误')}")
        return
    print("\n✅ GRPO训练结果:")
    ...
    if reward_data.get("status") != "success":
        print(f"❌ 奖励函数创建失败: {reward_data.get('message', '未知错误')}")
        return
    print("✅ 奖励函数创建成功:")
