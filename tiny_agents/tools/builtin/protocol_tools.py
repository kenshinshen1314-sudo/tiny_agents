...
class MCPTool(Tool):
    ...
    使用示例:
        >>> from tiny_agents.tools.builtin import MCPTool
        >>>
        >>> # 方式1: 使用内置演示服务器
    ...
    def _discover_tools(self):
        """发现MCP服务器提供的所有工具"""
        try:
            from tiny_agents.protocols.mcp.client import MCPClient
            import asyncio
...
class A2ATool(Tool):
    ...
    使用示例:
        >>> from tiny_agents.tools.builtin import A2ATool
        >>> # 连接到 A2A Agent（使用默认名称）
        >>> tool = A2ATool(agent_url="http://localhost:5123")
        >>> # 连接到 A2A Agent（自定义名称和描述）
        >>> tool = A2ATool(
        ...     agent_url="http://localhost:5123",
        ...     name="tech_expert",
    ...
    def run(self, parameters: Dict[str, Any]) -> str:
        ...
        """
        try:
            from tiny_agents.protocols.a2a.implementation import A2AClient, A2A_AVAILABLE
...
class ANPTool(Tool):
    ...
    使用示例:
        >>> from tiny_agents.tools.builtin import ANPTool
        >>> tool = ANPTool()
        >>> # 注册服务
        >>> result = tool.run({
    ...
    def __init__(self, name: str = "anp", description: str = None, discovery=None, network=None):
        ...
        from tiny_agents.protocols.anp.implementation import ANPDiscovery, ANPNetwork
    ...
    def run(self, parameters: Dict[str, Any]) -> str:
        ...
        Returns:
            操作结果
        """
        from tiny_agents.protocols.anp.implementation import ServiceInfo
