...
class MemoryTool(Tool):
    ...
    def execute(self, action: str, **kwargs) -> str:
        ...
        if action == "add":
            ...
            return f"不支持的操作: {action}。支持的操作: add, search, query_points, summary, stats, update, remove, forget, consolidate, clear_all"
    ...
    def _search_memory(
        ...
        try:
            ...
            # 使用提供的user_id或默认值
            target_user_id = user_id if user_id is not None else self.memory_manager.user_id
            ...
            if not results:
                ...
                # 添加诊断提示
                hint_parts.append(f"\n💡 搜索参数:")
                hint_parts.append(f"   - user_id: {target_user_id}")
                hint_parts.append(f"   - memory_types: {memory_types or '全部'}")
                hint_parts.append(f"   - min_importance: {min_importance}")

                hint_parts.append(f"\n💡 可能的原因:")
                hint_parts.append(f"   1. 数据库中没有相关数据")
                hint_parts.append(f"   2. user_id 不匹配（添加记忆时的 user_id 可能与查询时不同）")
                hint_parts.append(f"   3. memory_type 不匹配（请检查记忆类型）")

                hint_parts.append(f"\n💡 调试建议:")
                hint_parts.append(f"   - 运行: python demo/diagnose_memory.py")
                hint_parts.append(f"   - 或在查询时尝试不指定 user_id 参数")
