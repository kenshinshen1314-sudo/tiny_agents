...
用户输入任务后，系统会分析任务并推荐团队模板，最终由**用户人工确认**选择：
...
**权限控制原则：**
1. **最小权限** - Agent只能使用其角色必需的工具
2. **角色绑定** - 工具权限与角色强绑定，不可转让
3. **操作审计** - 所有工具调用记录操作日志
...
| 隔离级别 | 说明 | 实现方式 |
|----------|------|----------|
| **任务级隔离** | 不同任务的产出相互独立 | 每个任务独立目录 |
| **角色级隔离** | 角色间不可访问对方产出 | 目录权限控制 |
| **会话级隔离** | 同一任务的不同会话隔离 | 会话ID隔离 |
...
| 类型 | 示例 | 处理方式 |
|------|------|----------|
| **API密钥** | AWS密钥、数据库密码 | 环境变量，不存入代码/日志 |
| **个人隐私** | 姓名、电话、邮箱 | 脱敏处理或拒绝处理 |
| **业务机密** | 核心算法、内部架构 | 本地处理，不上传外部 |
| **认证凭证** | Token、Cookie | 内存处理，不持久化 |
...
**防护措施：**
1. **日志脱敏** - 输出日志时自动脱敏关键字段
2. **内存清理** - 任务完成后立即清理敏感数据
3. **外部禁止** - 禁止Agent调用外部不信任服务
...
**并发控制规则：**
1. **全局并发上限** - 同时执行的任务数 ≤ 10
2. **单任务并发上限** - 单任务内Agent并发 ≤ 5
3. **队列优先级** - 用户手动指定优先级
4. **超时释放** - Agent执行超时自动释放槽位
