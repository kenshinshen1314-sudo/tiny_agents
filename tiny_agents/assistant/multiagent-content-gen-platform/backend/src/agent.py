...
from prompts import (
    ...
    load_article_style_prompt,
)
from models import SummaryState, SummaryStateOutput, TodoItem
from services.planner import PlanningService
from services.reporter import ReportingService
from services.content_gen import ContentGenerationService
...
class DeepResearchAgent:
    ...
    def __init__(self, config: Configuration | None = None) -> None:
        ...
        # Content generation agent (自媒体内容创作)
        article_style_prompt = load_article_style_prompt()
        self.content_agent = self._create_tool_aware_agent(
            name="内容创作专家",
            system_prompt=article_style_prompt.strip(),
        )
        ...
        self.content_gen = ContentGenerationService(self.content_agent, self.config)
        ...
        for attempt in range(max_task_retries):
            try:
                ...
                if attempt == max_task_retries - 1:
                    # 所有重试都失败，记录失败状态
                    logger.error(f"Task {task.id} failed after {max_task_retries} attempts: {e}")
                    task.status = "failed"
                    if emit_stream:
                        yield {
                            "type": "task_status",
                ...
                logger.warning(
                    f"Task {task.id} retry {attempt + 1}/{max_task_retries}, "
                    f"waiting {wait_time}s: {e}"
                )
                if emit_stream:
                    yield {
                        "type": "status",
                        "message": f"任务 {task.id} 重试中 ({attempt + 1}/{max_task_retries})...",
                        "task_id": task.id,
                    }
